...
**Smart contract**
At a high level, smart contracts execute transaction(s) automatically when certain conditions are met. Think of them as an automated agreement, written in code, that tracks, verifies, and executes the binding transactions of a contract between various parties. Smart contracts were first proposed by Nick Szabo in 1996. Smart contracts are self-executing and reliable and do not require the actions or presence of third parties. The smart contract code is stored on, and distributed across, a decentralized blockchain network, making it transparent and irreversible.
...
**Stake pool**: a reliable block-producing server node that holds the combined stake of various stakeholders in a single entity, or pool.
