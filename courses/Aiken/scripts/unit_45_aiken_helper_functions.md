...
1. Where should you create supporting functions in an Aiken project?

A) In the validators folder
**B) In the lib folder**
C) In the modules folder
D) In the root directory

2. What keyword is used to define a function in Aiken?
...
3. What is a key characteristic of lists in Aiken?

A) They are mutable
B) They can contain elements of different types
**C) They are immutable**
D) They are unordered


4. What value does a valid test in Aiken return? (Assuming the test is not marked with ‘fail’ keyword)
...
5. What does the ? operator do in Aiken?

A) Signals an optional value
B) Marks a function as public
C) Indicates a recursive function
**D) Instructs the compiler to trace the expression if it evaluates to False**
