# Aiken Validator : Make a Reservation
...
- spend(datum: Option<Reservation>, action: Action, _output_reference: OutputReference, tx: Transaction) { ... }: This is the spend handler within the validator. It defines the logic that must be satisfied when funds are spent from an output locked by this validator script.
...
- when action is { ... }: This when expression checks the value of the action and performs different validations based on the action:
    - Cancel -> ...: If the action is Cancel: list.has(tx.extra_signatories, reservation.customer): This checks if the customer who made the reservation (identified in reservation.customer) is one of the signatories of the transaction (tx.extra_signatories). This ensures only the customer can cancel their own reservation.
