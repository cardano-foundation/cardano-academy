...
This is not possible on Cardano, however, as there is no interference from third-party code. Native assets are transferred more efficiently with minimal transaction fees. As user-defined assets enjoy the same status as ada at the protocol level, they are also very secure. The Cardano protocol has been stress-tested from daily use since 2017 on a blockchain with 100% uptime.

## Questions

1. What is the main advantage of Cardano's native asset implementation compared to
    Ethereum's token standards?
   
A)  It allows for more complex smart contract logic
B) It enables the creation of tokens with unlimited supply
**C) It reduces reliance on third-party smart contracts**
D) It supports the use of fractional tokens

2. Which of the following is a potential risk associated with Ethereum's token standards?
 
A) High transaction fees due to increased network congestion
**B) Freezing or blacklisting of user accounts by token issuers**
C) Inability to create non-fungible tokens (NFTs)
D) Limited functionality for user-defined tokens
