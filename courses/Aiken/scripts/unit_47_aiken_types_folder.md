...
1. What is the primary purpose of the Option type in Aiken?

**A) To represent a value that may or may not exist**
B) To define a function that quits immediately if it encounters the option keyword
C) To create a list with an unknown number of elements
D) To handle errors in a program

2. What does the todo keyword do in Aiken?

A) Halts program execution with an error message
**B) Indicates a section of code that needs to be completed later**
C) Defines a test case that is expected to fail
D) Indicates a syntax error

3. How does the fail keyword differ from the todo keyword?

A) fail produces a compiler warning, while todo does not
B) fail restarts your computer, while todo puts it to ‘sleep’ mode
C) fail is used in Haskell, while todo is used in Helios
**D) fail does not produce a compiler warning, while todo does**

4. What is the purpose of the @ " " notation with todo and fail?

A) To specify the type of the expression
**B) To provide a string message as context**
C) To suppress compiler warnings
D) To define a custom error message

5. Which of the following statements about Option, todo and fail is NOT true?

A) Option can hold any data type
B) todo is useful for marking incomplete code
C) fail is used to intentionally stop program execution
**D) String messages are always required for todo and fail**
