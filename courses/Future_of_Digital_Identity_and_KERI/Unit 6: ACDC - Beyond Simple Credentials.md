...
1. GLEIF (Root): The global root of trust.
2. QVI (Issuer): A "Qualified vLEI Issuer" accredited by GLEIF.
3. Legal Entity: The company itself.
4. OOR (Role): An "Official Organizational Role" credential held by an individual, for example, a CEO.
...
A. **Authentic Chained Data Containers**
B. Automated Credential Distribution Chain
C. Advanced Cryptographic Data Certificates
D. Autonomic Credential Delivery Channels

2. How are ACDCs structured?
A. **As Directed Acyclic Graphs (DAGs) with one-way links**
B. As flat, standalone PDF files
C. As circular reference chains
D. As simple key-value pairs stored in a database

3. How do ACDCs avoid “link rot” compared to standard W3C Verifiable Credentials?
A. They store all data on a central web server with guaranteed uptime
B. **They use Self-Addressing Identifiers (SAIDs) instead of web URLs for schema referencing**
C. They require annual renewal from the issuer
D. They use blockchain-hosted URLs that never change

4. What is the “killer feature” of ACDCs?
A. They can be printed on paper
B. They are free to issue and verify
C. **Data chaining – the ability to link credentials together to model provenance and delegation**
D. They work without any cryptography

5. In the vLEI example, what does the OOR credential represent?
A. A company’s financial audit report
B. A software license for the Veridian platform
C. A blockchain transaction receipt
D. **An Official Organisational Role held by an individual, such as a CEO**
